...
.care-panel {
    background:#ffffff; border:1px solid #dfe7f1; border-radius:8px;
    padding:1rem; margin-bottom:1rem;
}
.care-panel strong { color:#16365f; }
.status-note {
    background:#eef6ff; border-left:4px solid #2d7dd2;
    padding:.85rem 1rem; border-radius:6px; color:#17324d;
}
.risk-chip {
    display:inline-block; padding:.25rem .6rem; border-radius:999px;
    font-size:.78rem; font-weight:700; border:1px solid transparent;
}
.risk-high { background:#fde7e7; color:#8f1d1d; border-color:#f7c2c2; }
.risk-low { background:#e5f5ec; color:#176a38; border-color:#bfe5cd; }
.risk-neutral { background:#edf2f7; color:#425466; border-color:#d8e1eb; }
.risk-meter-card {
    background:#ffffff; border:1px solid #dfe7f1; border-radius:8px;
...
.report-summary h4 { margin:.15rem 0 .7rem; color:#15324f; }
.report-list { margin:.35rem 0; color:#34495e; }
.report-meta {
    color:#607086; font-size:.86rem; border-top:1px solid #edf2f7;
    padding-top:.75rem; margin-top:.9rem;
}
.map-frame {
    width:100%; height:510px; border:0; border-radius:8px;
    box-shadow:0 8px 24px rgba(22,54,95,.12);
}
.assistant-shell {
    border:1px solid #dfe7f1; border-radius:8px; padding:1rem;
    background:#ffffff;
}
.message-user, .message-assistant {
    border-radius:8px; padding:.75rem .9rem; margin:.5rem 0;
}
.message-user { background:#eef6ff; border-left:4px solid #2d7dd2; }
.message-assistant { background:#f7fafc; border-left:4px solid #1e5aa0; }
.upload-zone {
    background:#ffffff; border:1px solid #dfe7f1; border-radius:8px;
...
.risk-title { font-weight:700; color:#15324f; font-size:1rem; margin-bottom:.25rem; }
.risk-detail { color:#607086; font-size:.88rem; }
.clean-callout {
    border:1px solid #dfe7f1; border-left:4px solid #1e5aa0;
    background:#ffffff; border-radius:8px; padding:1rem; margin:1rem 0;
}
/* ── Dark mode ── */
...
:root {
    ...
    --accent:#2ba67a;
    --danger:#cf5f5f;
    --shadow:0 20px 45px rgba(17, 70, 78, 0.10);
}
html, body, [class*="css"], [data-testid="stAppViewContainer"] { font-family:'Manrope', sans-serif; color:var(--text); }
[data-testid="stAppViewContainer"] {
    background:
        radial-gradient(circle at top left, rgba(43,166,122,0.12), transparent 28%),
        radial-gradient(circle at top right, rgba(13,141,143,0.14), transparent 24%),
        linear-gradient(180deg, #f5fafb 0%, var(--bg) 100%) !important;
}
[data-testid="stHeader"] { background:transparent; }
[data-testid="stSidebar"] { background:linear-gradient(180deg, #0f2b38 0%, #123947 100%) !important; border-right:1px solid rgba(255,255,255,0.08); }
[data-testid="stSidebar"] * { color:#e7f3f6; }
[data-testid="stSidebar"] .stRadio label {
    border:1px solid rgba(231,243,246,0.08); background:rgba(255,255,255,0.04);
    border-radius:14px; margin-bottom:.35rem; padding:.35rem .45rem;
}
.block-container { padding-top:1.25rem; padding-bottom:2rem; }
.main-title { font-size:2.25rem; font-weight:800; color:#102f3b; margin-bottom:0.15rem; letter-spacing:-0.03em; }
.subtitle { color:var(--muted); margin-top:0; font-size:1rem; }
.metric-card, .feature-tile, .section-card, .upload-zone, .risk-row, .risk-meter-card, .report-summary, .assistant-shell, .care-panel, .clean-callout {
    background:rgba(255,255,255,0.88) !important; border:1px solid rgba(216,230,234,0.9) !important;
    border-radius:22px !important; box-shadow:var(--shadow) !important; backdrop-filter:blur(8px);
}
.metric-card { padding:1.1rem 1.25rem !important; background:linear-gradient(180deg, rgba(255,255,255,0.95), rgba(239,248,249,0.96)) !important; }
.metric-card:hover, .feature-tile:hover, .risk-row:hover { transform:translateY(-3px); box-shadow:0 24px 45px rgba(17,70,78,0.13) !important; }
.metric-card h2 { color:#103847; font-size:2rem; font-weight:800; }
.result-positive, .result-negative { padding:1.15rem 1.2rem; border-radius:18px; font-weight:700; border:1px solid transparent; }
.result-positive { background:#fff1f1 !important; color:#7d2a2a !important; border-color:#f1cdcd !important; }
.result-negative { background:#edf9f2 !important; color:#1c6742 !important; border-color:#ccebd8 !important; }
.sidebar-brand { text-align:center; padding:10px 0 4px; font-size:1.8rem; font-weight:800; color:#ffffff; letter-spacing:-0.04em; }
.sidebar-tagline { text-align:center; font-size:.72rem; color:#9fd0d6; margin-bottom:8px; letter-spacing:0.16em; text-transform:uppercase; }
.hero-banner {
    position:relative; overflow:hidden; background:linear-gradient(135deg, #103847 0%, #0d6f73 58%, #22a38b 100%) !important;
    color:white; padding:3rem 2.5rem; border-radius:28px; margin-bottom:1.75rem; box-shadow:var(--shadow);
}
.hero-banner::after {
...
.hero-banner h1 { font-size:2.7rem; font-weight:800; margin:0 0 .5rem; line-height:1.05; letter-spacing:-0.04em; }
.hero-banner p { font-size:1.02rem; opacity:.88; margin:0; max-width:680px; }
.hero-badge {
    display:inline-flex; gap:.45rem; align-items:center; background:rgba(255,255,255,.12);
    border:1px solid rgba(255,255,255,.18); color:white; padding:7px 14px; border-radius:999px;
    font-size:.78rem; margin-bottom:1rem; letter-spacing:.08em; text-transform:uppercase;
}
.feature-tile { padding:1.35rem; height:100%; transition:transform .18s ease, box-shadow .18s ease; }
.feature-icon {
    width:52px; height:52px; border-radius:16px; background:linear-gradient(135deg, rgba(13,111,115,0.12), rgba(43,166,122,0.18));
    display:flex; align-items:center; justify-content:center; font-size:1.2rem; font-weight:800; color:var(--primary); margin-bottom:.9rem;
}
.feature-title { font-weight:700; color:#163845; font-size:1rem; }
.feature-desc, .report-meta, .report-list, .risk-detail { color:var(--muted) !important; font-size:.9rem; line-height:1.55; }
.status-note {
    background:linear-gradient(180deg, #edf8fa, #f9fcfd) !important; border:1px solid #cbe5ea !important; border-left:5px solid var(--primary) !important;
    padding:1rem 1.1rem; border-radius:16px; color:#244654;
}
.risk-chip {
...
.risk-high { background:#fff0ef !important; color:#9f3535 !important; border-color:#f2c8c5 !important; }
.risk-low { background:#ecfaf2 !important; color:#216444 !important; border-color:#c8ead7 !important; }
.risk-neutral { background:#f3f7f9 !important; color:#4b6674 !important; border-color:#d9e5ea !important; }
.risk-row { padding:1.1rem 1.15rem; margin:.75rem 0; transition:transform .18s ease, box-shadow .18s ease; }
.risk-title { font-weight:800; color:#143541; font-size:1.02rem; margin-bottom:.25rem; }
.gauge-title { text-align:center; font-weight:700; color:#163845; margin-bottom:.35rem; font-size:.96rem; }
.clean-callout { padding:1rem 1.1rem; margin:1rem 0; border-left:5px solid var(--primary) !important; }
.upload-zone { padding:1.2rem 1.3rem; margin:.5rem 0 1rem; }
.report-workspace {
    background:linear-gradient(180deg, rgba(255,255,255,0.74), rgba(247,252,253,0.92));
    border:1px solid rgba(216,230,234,0.95); border-radius:28px; padding:1.2rem; box-shadow:var(--shadow); margin-bottom:1.4rem;
}
.analysis-hero { display:grid; grid-template-columns:2fr 1fr; gap:1rem; margin-bottom:1rem; }
.analysis-panel {
    background:linear-gradient(135deg, #123947 0%, #0d6f73 62%, #1a9383 100%);
    color:white; border-radius:24px; padding:1.4rem 1.5rem; box-shadow:var(--shadow);
}
.analysis-panel h3 { margin:0 0 .45rem; font-size:1.6rem; font-weight:800; }
.analysis-panel p { margin:0; opacity:.88; line-height:1.55; }
.analysis-stat {
    background:rgba(255,255,255,0.84); color:#123947; border-radius:24px; padding:1.25rem;
    border:1px solid rgba(216,230,234,0.9); box-shadow:var(--shadow);
}
.analysis-stat .label { color:var(--muted); font-size:.82rem; text-transform:uppercase; letter-spacing:.08em; }
.analysis-stat .value { font-size:2rem; font-weight:800; margin-top:.35rem; }
.clinical-card {
    background:linear-gradient(180deg, rgba(255,255,255,0.96), rgba(243,249,250,0.98));
    border:1px solid rgba(216,230,234,0.92); border-radius:24px; padding:1.2rem 1.25rem; box-shadow:var(--shadow); margin-top:1rem;
}
.clinical-heading { font-size:1.15rem; font-weight:800; color:#153542; margin-bottom:.65rem; }
.clinical-summary { color:#35525f; line-height:1.75; font-size:.97rem; }
.signal-grid { display:grid; grid-template-columns:repeat(auto-fit, minmax(220px,1fr)); gap:.85rem; margin:1rem 0; }
.signal-card {
    padding:1rem; border-radius:20px; border:1px solid #d7e6ea; background:linear-gradient(180deg, #ffffff 0%, #f5fafb 100%);
}
.signal-card.high { border-color:#f0c9c6; background:linear-gradient(180deg, #fff8f7 0%, #fff1ef 100%); }
.signal-card .signal-name { font-size:.95rem; font-weight:800; color:#173744; margin-bottom:.35rem; }
.signal-card .signal-score { font-size:1.6rem; font-weight:800; color:#0d6f73; margin-bottom:.3rem; }
.signal-card.high .signal-score { color:#b44747; }
.signal-card .signal-note { color:var(--muted); font-size:.87rem; line-height:1.55; }
div[data-testid="stFileUploader"] section { border:2px dashed #a8cfd1 !important; border-radius:20px; background:#f7fcfd; }
.stButton > button, .stDownloadButton > button {
    border-radius:16px !important; border:1px solid rgba(13,111,115,0.15) !important;
    background:linear-gradient(135deg, #0d6f73, #1a9383) !important; color:white !important;
    font-weight:700 !important; box-shadow:0 12px 24px rgba(13,111,115,0.18);
}
.stButton > button:hover, .stDownloadButton > button:hover { filter:brightness(1.03); transform:translateY(-1px); }
[data-testid="stAppViewContainer"].dark-mode { background:#09141b !important; }
[data-testid="stSidebar"].dark-mode { background:#071219 !important; }
.dark-mode .main-title, .dark-mode .feature-title, .dark-mode .gauge-title, .dark-mode .clinical-heading { color:#dcf5f3 !important; }
.dark-mode .subtitle, .dark-mode .feature-desc, .dark-mode .clinical-summary, .dark-mode .risk-detail, .dark-mode .report-meta { color:#9eb7bf !important; }
.dark-mode .metric-card, .dark-mode .feature-tile, .dark-mode .section-card, .dark-mode .upload-zone, .dark-mode .risk-row, .dark-mode .risk-meter-card,
.dark-mode .report-summary, .dark-mode .assistant-shell, .dark-mode .care-panel, .dark-mode .clean-callout, .dark-mode .clinical-card, .dark-mode .signal-card,
.dark-mode .analysis-stat, .dark-mode .report-workspace { background:#0f2028 !important; color:#e8f5f4 !important; border-color:#21424d !important; }
.dark-mode .signal-card.high { background:#281617 !important; border-color:#7c3d40 !important; }
@media (max-width: 980px) {
    .analysis-hero { grid-template-columns:1fr; }
    .hero-banner { padding:2rem 1.35rem; }
...
def auth_screen():
    ...
    with col2:
        ...
        <div class="hero-banner">
            <div class="hero-badge">Patient Health Intelligence</div>
            <h1>Medical Report<br>Risk Analyzer</h1>
            <p>Upload lab reports, extract values, estimate risk signals, and track patient progress.</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        for col, (icon, title, desc) in zip([f1,f2,f3], [
            ("01","Report Upload","PDF, JPG, JPEG, and PNG support"),
            ("02","Value Extraction","Reads lab markers and flags missing data"),
            ("03","Risk Tracking","Stores results for follow-up comparison"),
        ]):
...
def sidebar() -> str:
    ...
    "Dashboard", "About", "Report Analyzer", "Saved Reports",
...
def page_disease_prediction():
    ...
    st.markdown('<p class="subtitle">Use this when a report is not available. Screening output is generated by Groq from submitted clinical values.</p>',
                unsafe_allow_html=True)
    disease      = st.selectbox("Select clinical form", list(SCHEMAS.keys()))
    _, fields = get_predictor(disease)
    api_key = get_groq_api_key()
    if not api_key:
        st.warning("Groq API key is missing. Add it in `.streamlit/secrets.toml` under `[groq] api_key`.")
...
def page_report_analyzer():
        ...
        <div class="report-workspace">
            <div class="analysis-hero">
                <div class="analysis-panel">
                    <div class="hero-badge">Hospital Style Workflow</div>
                    <h3>Report intake, extraction, and clinical triage</h3>
                    <p>The backend reads the uploaded report, extracts laboratory markers, evaluates disease patterns, and returns a professional result summary in the report section.</p>
                </div>
                <div class="analysis-stat">
                    <div class="label">Focus</div>
                    <div class="value">Multi-disease confidence</div>
                    <div class="report-meta">High-confidence disease patterns are highlighted automatically.</div>
                </div>
            </div>
        </div>
        """,
    ...
    api_key = get_groq_api_key()
    if not api_key:
        st.warning("Analysis service key is missing. Add it in `.streamlit/secrets.toml` under `[groq] api_key` to enable the report analysis engine.")
    ...
    if analyze and uploaded is not None:
        ...
        try:
            analysis = (
                groq_report_analysis(extracted_text, values, risks, missing, patient_context, api_key)
                if api_key else
                fallback_report_analysis(risks, missing, overall, values)
            )
        except Exception as exc:
            st.warning(f"Groq analysis was unavailable, so the app used the local fallback summary. Details: {exc}")
        ...
        rid = save_report(
            ...
            uploaded.name,
            prediction_label,
            confidence=float(analysis.get("overall_risk", overall)) / 100,
            severity=float(analysis.get("overall_risk", overall)) / 10,
            details=json.dumps(details, default=str),
        )
        st.success(f"Analysis saved to reports. Report ID: {rid}")
...
def page_chatbot():
    ...
    st.markdown('<p class="subtitle">Ask health questions and get educational guidance powered by Groq.</p>',
                unsafe_allow_html=True)
    ...
    api_key = get_groq_api_key()
    if not api_key:
        st.warning("Groq API key is missing. Add it in `.streamlit/secrets.toml` or Streamlit Cloud Secrets under `[groq] api_key`.")

    with st.form("health_assistant_form", clear_on_submit=True):
        user_input = st.text_area(
            "Your question",
            placeholder="Example: What symptoms of diabetes should I watch for?",
            height=110,
        )
        submitted = st.form_submit_button("Ask assistant", use_container_width=True, disabled=not api_key)

    st.markdown('<div class="assistant-shell">', unsafe_allow_html=True)
    if not st.session_state.chat_history:
        st.markdown('<div class="status-note">Start with a symptom, report result, or prevention question. The assistant provides education, not diagnosis.</div>',
                    unsafe_allow_html=True)
    ...
    if submitted and user_input.strip() and api_key:
        ...
        if looks_non_medical and not looks_medical:
            st.session_state.chat_history.append({
                "role": "assistant",
                "content": (
                    "I can only help with medical and health-related questions in this app. "
                    "Please ask about symptoms, prevention, tests, medicines, or when to seek care."
                ),
            })
            st.rerun()
                    ...
                    messages=[
                        {
                            ...
                            "content": (
                                ...
                                "If the user asks anything unrelated to health/medicine, refuse briefly and ask for a medical question. "
                                "For medical questions: give concise, patient-friendly education, include red-flag symptoms and when to seek urgent care, "
                                "and remind users to consult a qualified doctor. Do not diagnose."
                            )
                        },
    ...
    st.caption("This assistant is for education only. Always consult a qualified doctor for medical advice.")
...
# ── Router ────────────────────────────────────────────────────────────────────
def page_about():
    st.markdown('<p class="main-title">About MDPS</p>', unsafe_allow_html=True)
    st.markdown('<p class="subtitle">A report-first patient screening tool for extracting lab values and tracking risk signals over time.</p>',
                unsafe_allow_html=True)
    st.markdown("""
    <div class="clean-callout">
        MDPS now focuses on medical report understanding. Instead of asking the patient to choose a disease first,
        the app reads uploaded reports, extracts measurable values, flags missing data, and produces a transparent
        risk summary for follow-up discussion with a clinician.
    </div>
    """, unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    c1.markdown('<div class="feature-tile"><div class="feature-icon">01</div><div class="feature-title">Report Intake</div><div class="feature-desc">Accepts PDF reports and extracts readable lab values locally.</div></div>', unsafe_allow_html=True)
    c2.markdown('<div class="feature-tile"><div class="feature-icon">02</div><div class="feature-title">Value Verification</div><div class="feature-desc">Shows extracted values, units, ranges, and missing fields so false or incomplete values can be checked.</div></div>', unsafe_allow_html=True)
    c3.markdown('<div class="feature-tile"><div class="feature-icon">03</div><div class="feature-title">Patient Tracking</div><div class="feature-desc">Saves each analysis and charts progress across future reports for the same patient.</div></div>', unsafe_allow_html=True)
    st.markdown("### Clinical disclaimer")
    st.warning("This tool is for screening and education only. It does not diagnose disease or replace a qualified doctor.")
